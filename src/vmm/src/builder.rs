...
fn attach_block_devices(
    ...
    for drive_config in vm_resources.block.config_list.iter() {
        ...
        attach_block_device(
            ...
            MmioTransport::new(vmm.guest_memory().clone(), block_device.clone()).map_err(|e| {
...
fn attach_net_devices(
    ...
    for cfg in vm_resources.network_interface.iter() {
        ...
        attach_mmio_device(
            vmm,
            cfg.iface_id.clone(),
            MmioTransport::new(vmm.guest_memory().clone(), net_box).map_err(|e| {
                RegisterMMIODevice(super::device_manager::mmio::Error::CreateMmioDevice(e))
            })?,
        )?;
    }
...
fn attach_vsock_device(
    ...
    if let Some(cfg) = vm_resources.vsock.as_ref() {
        ...
        attach_mmio_device(
            vmm,
            cfg.vsock_id.clone(),
            MmioTransport::new(vmm.guest_memory().clone(), vsock_box).map_err(|e| {
